...
const Header = ({ lang, setLang }: HeaderProps) => {
  ...
  const scrollToSection = (sectionNumber: number) => {
    ...
    if (window.location.pathname !== '/') {
      // If not on home page, navigate to home with hash for direct section targeting
      let sectionHash = '';
      switch (sectionNumber) {
        case 1:
          ...
          break;
        case 2:
          sectionHash = '#how-it-works';
          break;
        case 3:
          sectionHash = '#business-solutions';
          break;
      }
        ...
        <div className="flex items-center justify-between bg-[#1a1a1a]/95 backdrop-blur-xl rounded-full px-6 py-3 shadow-2xl border border-white/10 min-w-[320px] max-w-6xl">
          ...
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            <div className="hidden md:flex items-center gap-2">
              ...
              <Link 
                href="/about"
                className="flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5 text-xs text-white/80 border border-white/20 hover:bg-white/20 transition-all duration-300 cursor-pointer"
              >
              ...
              <nav className="flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-2 pt-4 border-t border-white/20">
                  <Link
                    href="/about"
                    onClick={() => setIsMenuOpen(false)}
                    className="flex items-center justify-center gap-2 rounded-full bg-white/10 px-4 py-2 text-xs text-white/80 border border-white/20 hover:bg-white/20 transition-all duration-300 cursor-pointer"
                  >
                    <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                      <path d="M3,20.5V3.5C3,2.91 3.34,2.39 3.84,2.15L13.69,12L3.84,21.85C3.34,21.61 3,21.09 3,20.5M16.81,15.12L6.05,21.34L14.54,12.85L16.81,15.12M20.16,10.81C20.5,11.08 20.75,11.5 20.75,12C20.75,12.5 20.53,12.92 20.18,13.18L17.89,14.5L15.39,12L17.89,9.5L20.16,10.81M6.05,2.66L16.81,8.88L14.54,11.15L6.05,2.66Z"/>
                    </svg>
